...
def antinode_locations(city_map: List[str], verbose: bool = False) -> int:
    ...
    def in_bounds(position: Coord) -> bool:
    ...
    for frequency in antennae:
        for A, B in itertools.combinations(antennae[frequency], 2):
            diff = A - B
            A_line = [A]  # A + diff
            while in_bounds(A_line[-1]):
                A_line.append(A_line[-1] + diff)
            antinodes[frequency].update(set(A_line))
            B_line = [B]  # B - diff
            while in_bounds(B_line[-1]):
                B_line.append(B_line[-1] - diff)
            antinodes[frequency].update(set(B_line))

    # if verbose:
    #     for frequency in antinodes:
    #         print(f"{len(antinodes[frequency])} {frequency} antinodes")
    ...
    antinodes = {
        ...
        if in_bounds(position)}  # still some to trim
    # result
    if verbose:
